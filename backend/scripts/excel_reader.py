...
def create_object_type_only_pivot_table():
    ...
    with pd.ExcelWriter(get_output_file('VEHICLE_REPLACEMENT_BY_CATEGORY')) as writer:
        object_type_pivot.to_excel(writer, sheet_name='ObjectType_Summary', index=False)
    
    print(f"\nObjectType-Only Vehicle Replacement analysis saved to: {get_output_file('VEHICLE_REPLACEMENT_BY_CATEGORY')}")
...
def create_vehicle_replacement_hierarchical_pivot_table():
    ...
    with pd.ExcelWriter(get_output_file('VEHICLE_REPLACEMENT_DETAILED_FORECAST')) as writer:
        hierarchical_pivot.to_excel(writer, sheet_name='Hierarchical_Summary', index=False)
    
    print(f"\nHierarchical Vehicle Replacement analysis saved to: {get_output_file('VEHICLE_REPLACEMENT_DETAILED_FORECAST')}")
